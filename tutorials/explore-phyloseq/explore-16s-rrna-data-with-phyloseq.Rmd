...
An important concept in microbiome data analysis is the concept of data **normalization**. Data normalization corrects for differences in sample size and allows us to make comparisons between samples. Not every analysis will use normalized reads as an input, but when necessary, our ASV counts were normalized by the total number of reads in each sample to convert the counts to proportion.
...
Let's explore our phyloseq object (miso). We can check by calling the object through its name. Do so now with the code below.
...
<p class=caption> **The phyloseq class**: The image above shows a conceptual diagram of the phyloseq class. All phyloseq objects will share the characteristics of the class. Dotted lines between the tables show how phyloseq is able to relate the different data in each tables to each other. The sample_table and otu_table both have samples stored in rows, but the tax_table has ASVs stored in rows. Image by Sayumi York (March 21, 2025).</p>

Our phyloseq object is composed of 3 core data tables (some have more).

  - **The sample data table (sample_data)**: which holds all the metadata relating to each sample. For example in this case our metadata contains information on what timepoint in the experiment the sample is from, the gender of the subject, whether they were on the homogenized diet at the time, the subject's age, etc.
  
  - **The OTU abundance table (otu_table)**: which holds the data on how many of each ASV were found in each sample.
  
  -  **The taxonomic annotation table (tax_table)**: which holds all the taxonomic data for each ASV (what phylum, class, order, etc. the ASV has belongs to). Not every ASV has a taxonomic assignment at every level.
...
Note: The sample data is stored as a data frame class (or table) - as opposed to the matrix class for example - sample data can be called using a data.frame() command to display the sample data as an interactive table.
...
Let's try looking at different variables for a given column of metadata. Change the column name in between the double quotes to check different columns. We've given you two examples.
