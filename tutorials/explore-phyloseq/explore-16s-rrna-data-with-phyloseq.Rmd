...
quiz(caption = "What's in the phyloseq object?",
  ...
  question("How many ASVs are in the phyloseq object?",
    ...
    answer("9"),
    allow_retry = TRUE,
    try_again = "HINT: ASVs represent taxa. How many taxa are in the phyloseq object?",
  ...
  question("How many variables are there in the experiment? (ex. age, gender, timepoint)",
    answer("1702"),
    answer("105"),
    answer("28", correct = TRUE),
    answer("9"),
    allow_retry = TRUE,
    try_again = "HINT: The variables are called sample_variables in the phyloseq object.",
    random_answer_order = TRUE
  )
)
```
...
quiz(caption = "Exploring the sample_table",
  ...
  question("Check all that are variables in the sample_data",
    ...
    answer("diet", correct=TRUE),
    answer("timepoint", correct=TRUE),
    try_again = "HINT: Find this by exploring the table created by the head() and data.frame() commands using the arrows in the header row. Alternatively, we can look all the sample variables in the sample_data table using the column names from the code: (colnames(sample_data(miso)))",
    allow_retry = TRUE
  )
)
```

Let's try looking at different metadata variables (e.g. subject and diet) represented by the columns of the metadata table. 

Use code block below to identify unique subjects and diets present in miso data. Try substituting the column name in between the double quotes with another metadata variable (e.g. age). 

```{r unique_metadata, exercise=TRUE, echo=FALSE}
unique(data.frame(sample_data(miso)[, "subject"]))
unique(data.frame(sample_data(miso)[, "diet"]))
...
quiz(caption="Exploring metadata in sample_table",
  ...
  question("How many subjects are in the dataset?",
    answer("5"),
    answer("21", correct=TRUE),
    answer("35"),
    answer("105"),
    answer("32"),
    try_again = "HINT: This is equal to the number of rows in the results table from using the unique data.frame code on the subject variable.",
    allow_retry = TRUE
  ),
  
  question("What are the diets in the dataset?",
    answer("1-5"),
...
quiz(caption = "What's in the otu_table?",
    ...
    try_again = "HINT: Modify the above code to look at a range of the first 5 samples for ASV1000.",
    allow_retry = TRUE
  ),
  ...
  question("What is the normalized count for ASV115 in sample 10",
    ...
    try_again = "HINT: We can either ask specifically for sample 10 or plot the first 10 samples for ASV115.",
    allow_retry = TRUE
  )
)
```
...
quiz(caption = "Let's find a taxnomic assignment by ASV",
  question("What is the taxonomic Family associated with ASV2005?",
    ...
    answer("NA"),
    try_again = "HINT: Modify the above code to look at ASV2005 rather than ASV1.",
    allow_retry = TRUE
  )
)
```
...
quiz(caption = "Plotting phylum by subject",
  question("How many different known Phyla are in the data?",
    answer("14", message = "HINT: We asked for KNOWN phyla, so don't include NA"),
...
plot_bar(miso, x= "subject", fill = "Phylum", title = "My title") +
  geom_bar(aes(color = Phylum, fill = Phylum), stat = "identity", position = "stack")
```
```{r plot_bar_subject_Class-hint-1}
# Change all instances of Phylum to Class.
# Remember to use the same capitalization style.
```

Try plotting different taxonomy based on another variable such as diet, age, and gender, by substituting "subject".

```{r plot_bar_subject_Phylum, exercise=TRUE, exercise.eval=FALSE}
plot_bar(miso, x= "subject", fill = "Phylum", title = "My title") +
  geom_bar(aes(color = Phylum, fill = Phylum), stat = "identity", position = "stack")
```
```{r plot_bar_subject_Phylum-hint-1}
# To change what variable we compare samples between, we change what is on the x-axis in plot_bar. Try to change x = "subject" to x = "diet".
```
---

```{r plot_bar_tax_quiz}
quiz(caption = "Changing the phyloseq graph by taxonomy",
    ...
    answer("Mollicutes"),
    answer("Synergistia"),
    try_again = "HINT: Did you remember to change all instances of Phylum to Class? Did you use the correct capitalization?",
    allow_retry = TRUE,
    random_answer_order = TRUE
  ),
  ...
  question("What timepoint has the lowest proportion of Verrucomicrobia?",
    ...
    incorrect = "HINT: Verrucomicrobia is a phylum. We must plot phylum against the *timepoint* variable on the x-axis.",
    allow_retry = TRUE,
...
plot_bar(miso, x= "subject", fill = "Order", title = "My title") +
...
plot_bar(my_subset, x="subject", fill = "Phylum", title = "My title") +
  geom_bar(aes(color = Phylum, fill = Phylum), stat = "identity", position = "stack")

```
```{r subsetting_bacteroidia-hint-1}
# Bacteroidia is a class, so in the my_subset line we'll want to swap Phylum for Class
```
```{r subsetting_bacteroidia-hint-2}
# In the my_subset line, We want to swap out Firmicutes for Bacteroidia
```
```{r subsetting_bacteroidia-hint-3}
# We want to subset by class but plot by Order
```
```{r subsetting_bacteroidia-hint-4}
# Change all instances of Phylum in the plot_bar code to Order
```
```{r subsetting_bacteroidia-hint-5}
# Check that all your capitalization is the same as the original code
```
```{r subsetting_bacteroidia-hint-6}
# Change the title of the plot to your own
```
...
quiz(caption = "Changing the phyloseq graph",
  question("How many different known classes are represented in the entire dataset?",
    ...
    answer("14"),
    answer("21", correct=TRUE),
    try_again = "HINT: We want to look at the entire dataset, so we won't be using any code to subset. Instead of plotting by Phylum, plot by Class (plot_bar)",
    allow_retry = TRUE
  ),
  ...
  question("What are the 2 most abundant orders in the class Gammaproteobacteria?",
    ...
    allow_retry = TRUE,
    try_again = "HINT: We will need to subset by the class Gammaproteobacteria (subset_taxa step) and plot by order (plot_bar step)."
  )
)
```
...
plot_bar(miso, x="timepoint", fill = "Phylum", title = "My title") +
  geom_bar(aes(color = Phylum, fill = Phylum), stat = "identity", position = "stack") +
  facet_wrap(~subject)
```
