...
output:
  learnr::tutorial:
    ...
    version: 1.0
subtitle: |
  ![](images/cmoor_logo_notext.png){width=50%}
...
1. What variable is represented by the shape of the data points
...
```
## Group exercise: Project ideas

Let's think about how students would use these tools to build an independent research project!

As a reminder, we have the following metadata variables available to us:
...
We have yet to use the gender variable. Together with others nearby tackle one of the following of your choice:

1. Come up with a **research question** that includes gender and create 1-2 plots that answer your question. 

2. **Compare at least two variables** to determine which has a stronger effect on the gut microbiome. You will need to make at least 2 plots to answer this question.

3. **Choose a taxa** of interest that you've seen in one of your plots. Look up some *scientific articles* to figure out how might this taxa relate to human health or your research question (ex. why might we see it more in one gender than another?)

We've provided some template code blocks for you to work here:

```{r plot_bar_demo_group1, exercise=TRUE, echo=FALSE}
plot_bar(miso, x="subject", fill = "Phylum",) +
  geom_bar(aes(color = Phylum, fill = Phylum), stat = "identity", position = "stack")
```


```{r plot_bar_demo_group2, exercise=TRUE, echo=FALSE}
```

```{r plot_bar_demo_group3, exercise=TRUE, echo=FALSE}
```

```{r alpha_diversity_sampler_group1, exercise=TRUE, echo=FALSE}
...
```{r alpha_diversity_sampler_group2, exercise=TRUE, echo=FALSE}

```

```{r alpha_diversity_sampler_group3, exercise=TRUE, echo=FALSE}
