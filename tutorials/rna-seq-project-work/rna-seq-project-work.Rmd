...
image: "![](images/rosa_and_students.jpg)"
image_caption: ""
author:
  - "Sayumi York, Notre Dame of Maryland University"
  - "Gauri Paul, Clovis Community College"
learning_goals:
  - "Familiarize the RNA-seq datasets available to you"
...
#Check if we are on SciServer by seeing if the file path to marianes-regions on SciServer is available.
file_path <- "/home/idies/workspace/c_moor_data/marianes-regions/htseq/"
if (file.exists(file_path)) {

#MarianesMidgutData
  midgut_tsv <- read.table("/home/idies/workspace/c_moor_data/marianes-regions/htseq/midgut.tsv", header=TRUE)
  midgut <- readRDS("~/workspace/c_moor_data/instructors/preprocessed_data/marianes/marianes_htseq_DESeq.rds")
  R1_vs_R2_clusters <- readRDS("~/workspace/c_moor_data/instructors/preprocessed_data/marianes/a1_vs_p1_clusters.rds")

# SharonGutbrainData
  gutbrain<-readRDS("/home/idies/workspace/c_moor_data/sharon-brain/gutbrain.RDS")
  gutbrain_metadata<-read.csv("/home/idies/workspace/c_moor_data/sharon-brain/gutbrain_metadata.csv", row.names = 1)

# SchaumOrganData
  heart<-readRDS("/home/idies/workspace/c_moor_data/schaum-organs/Schaum_Heart.RDS")
  kidney<-readRDS("/home/idies/workspace/c_moor_data/schaum-organs/Schaum_Kidney.RDS")
  brain<-readRDS("/home/idies/workspace/c_moor_data/schaum-organs/Schaum_Brain.RDS")
  
  heart_metadata<-read.delim("/home/idies/workspace/c_moor_data/schaum-organs/Schaum_Metadata_Heart.tsv", row.names=1)
  kidney_metadata<-read.delim("/home/idies/workspace/c_moor_data/schaum-organs/Schaum_Metadata_Kidney.tsv", row.names=1)
  brain_metadata<-read.delim("/home/idies/workspace/c_moor_data/schaum-organs/Schaum_Metadata_Brain.tsv", row.names=1)
  
} else {
  #If not, assume we are on AnVIL and load in the data through packages
  library("MarianesMidgutData")
  R1_vs_R2_clusters <- MarianesMidgutData::a1_vs_p1_clusters
  library("SharonBrainData")
  library("SchaumOrganData")
}
...
**1. Run a clusterProfiler analysis on two regions.**: Jump to the clusterProfiler analysis section and choose two sections of the midgut to run the analysis on. From within your dotplot results, choose a category with a with low *padj* value and high count value. 
...
**4. Craft an engaging biological story from your genes**: This story should focus on the category you chose when looking at your clusterProfiler dot plot. How might this pathway be connected to the model organism and/or the samples you are looking at? What is important about this pathway and how might disruption cause atypical function in the body?
...
raw_results <- results(midgut, contrast = c("condition", "REGION1", "REGION2") )
R1_vs_R2 <- formatDESeq2Results(raw_results)
...
### Get differential expression results for a single gene in the Drosophila midgut dataset

```{r filter-by-name-drosophila, exercise=TRUE, echo=FALSE}
raw_results <- results(midgut, contrast = c("condition", "REGION1", "REGION2") )
R1_vs_R2 <- formatDESeq2Results(raw_results)

my_gene <- filter(R1_vs_R2, GeneID=="FBgn#######")
my_gene
```

```{r filter-by-name-drosophila_tryit_1, exercise=TRUE, echo=FALSE}

```
```{r filter-by-name-drosophila_tryit_2, exercise=TRUE, echo=FALSE}

```
```{r filter-by-name-drosophila_tryit_3, exercise=TRUE, echo=FALSE}
...
## Meet the organ dataset

To best make use of this data we recommend you complete the following modules if you have yet to do so:

- Introduction to RNA-seq data
- Differential expression with DESeq2

These data come from [**Aging hallmarks exhibit organ-specific temporal signatures** by Schaum et al. 2020](https://pmc.ncbi.nlm.nih.gov/articles/PMC7757734/). In this study, the research team sampled bulk RNA-seq data of different organ systems in mice every 3 months, ranging from 1 month to 27 months in age. Each time point from month 1 to 21 contained 4 male mice and 2 female mice; only the male mice were sampled at 24 and 27 months.

![](images/Schaum_Study_Design.png){width=100%}

Through this data, we can learn more about how gene expression changes with age. As you can imagine, RNA-seq data from 17 different organ systems is quite large. We have presented you with just 3 to focus on:

- The brain
- The heart
- The kidney

**The differential expression analysis available here and resulting DESeq2 object are identical to those from the original study which used age as a categorical value and sex as covariates (~age_categorical + sex) to look at differential gene expression across different ages.**

In addition to our analyses here, at the time of writing this module, the original authors have provided a [shiny app](https://twc-stanford.shinyapps.io/maca/) that allows you to explore a single gene across all 17 sampled organ systems. We hope that you can use these tools in tandem with what we have provided here to create a great research project!

### Working with mouse functions and resources

Our standard RNA-seq miniCURE modules use the *Drosophila* dataset and have accompanying resources such as FlyBase. When working with mouse data, we will have to use mouse databses and functions.
...
- [**Mouse Genome Informatics (MGI)**](https://www.informatics.jax.org/) is the database you should use instead of FlyBase. MGI uses the Ensembl Gene Model ID format, where mouse genes take the format ENSMUSG###########. Of course, since they're in different formats, you will get no results if you search for a fly gene in MGI or a mouse gene in FlyBase.

- **Use the specialized functions for the organ dataset**: For our DESeq2 analysis, please remember to look carefully at what organ you are working with at one time. We have a separate function for each organ available for plotting gene expression across mice of different ages and sex. These functions include: plotAcrossBrain, plotAcrossHeart, and plotAcrossKidney. These functions work the same way as the Drosophila functions, except they take an MGI ID instead of a FlyBase ID.


### Some research ideas to get you started

- Consider some disease states that are linked with aging, such as dementia, heart disease, and kidney disease. What genes have other people found are linked to these diseases, and is there any correlation of these genes across multiple organs? For example, does increased gene expression in one organ correlate with increased gene expression in another or vice versa?
- Early development has mammals growing at a much faster rate before tapering off and eventually stopping. This growth period includes an increase in body size, the development and maturation of reproductive systems, and maturation of physical and mental systems. What genes are involved in these changes and do they taper off or are they sustained over time?
- While aging is a natural process, cellular systems are continuously repairing DNA damage, replenishing cells (in certain organ systems), and mitigating the effects of aging on the body. What are the differences in these processes across organs, and does gene expression of these repair processes change over time?


### Differential expression template (Mouse organ dataset)


:::{.attention_box_pause}
:::{.attention_header_box_pause}
:::{.attention_title}
#### **Pay attention to which organ you're working with!**
:::
:::
::: {.attention_content}
There are minor differences in the template code boxes in the following sections which correspond to analyses of the brain, the heart, and the kidney. It can be easy to mix up pieces of code as you work, so make sure you're working with the right code template!
:::
:::

```{r deseq2-brain-one-pager, exercise=TRUE, echo=FALSE}
#FOR THE BRAIN
temp <- results(brain, contrast = c("age", "1", "27") )
...
```{r deseq2-heart-one-pager, exercise=TRUE, echo=FALSE}
#FOR THE HEART
temp <- results(heart, contrast = c("age", "1", "27") )
R1_vs_R2 <- formatDESeq2Results(temp)
sig_genes <- filter(R1_vs_R2, padj <= 0.05)
dim(sig_genes)
sig_genes
...
```{r deseq2-kidney-one-pager, exercise=TRUE, echo=FALSE}
#FOR THE KIDNEY
temp <- results(kidney, contrast = c("age", "1", "27") )
R1_vs_R2 <- formatDESeq2Results(temp)
sig_genes <- filter(R1_vs_R2, padj <= 0.05)
dim(sig_genes)
sig_genes
...
```{r deseq2-organs_tryit1, exercise=TRUE, echo=FALSE}
#WHAT ORGAN ARE YOU WORKING ON?
```
```{r deseq2-organs_tryit2, exercise=TRUE, echo=FALSE}
#WHAT ORGAN ARE YOU WORKING ON?
```
```{r deseq2-organs_tryit3, exercise=TRUE, echo=FALSE}
#WHAT ORGAN ARE YOU WORKING ON?
```

### Get differential expression results for a single gene in the mouse brain dataset

```{r filter-by-name-BRAIN, exercise=TRUE, echo=FALSE}
#FOR THE BRAIN
temp <- results(brain, contrast = c("age", "1", "27") )
R1_vs_R2 <- formatDESeq2Results(temp)
my_gene <- filter(R1_vs_R2, GeneID=="ENSMUSG###########")
my_gene
```

```{r filter-by-name-HEART, exercise=TRUE, echo=FALSE}
#FOR THE HEART
temp <- results(heart, contrast = c("age", "1", "27") )
R1_vs_R2 <- formatDESeq2Results(temp)
my_gene <- filter(R1_vs_R2, GeneID=="ENSMUSG###########")
...
```{r filter-by-name-KIDNEY, exercise=TRUE, echo=FALSE}
#FOR THE KIDNEY
temp <- results(kidney, contrast = c("age", "1", "27") )
R1_vs_R2 <- formatDESeq2Results(temp)
my_gene <- filter(R1_vs_R2, GeneID=="ENSMUSG###########")
my_gene
```
```{r filter-by-name-ORGAN_tryit-1, exercise=TRUE, echo=FALSE}
#WHAT ORGAN ARE YOU WORKING ON?

```
```{r filter-by-name-ORGAN_tryit-2, exercise=TRUE, echo=FALSE}
#WHAT ORGAN ARE YOU WORKING ON?

```
```{r filter-by-name-ORGAN_tryit-3, exercise=TRUE, echo=FALSE}
#WHAT ORGAN ARE YOU WORKING ON?

```


### clusterProfiler for the organ datasets

```{r clusterprofiler_organ_brain, exercise=TRUE, echo=FALSE}
#BRAIN
# Run differential expression analysis
temp <- results(brain, contrast = c("age", "1", "27") )
R1_vs_R2 <- formatDESeq2Results(temp)
...
```
```{r clusterprofiler_organ_heart, exercise=TRUE, echo=FALSE}
#HEART
# Run differential expression analysis
temp <- results(heart, contrast = c("age", "1", "27") )
R1_vs_R2 <- formatDESeq2Results(temp)
...
```{r clusterprofiler_organ_kidney, exercise=TRUE, echo=FALSE}
#KIDNEY
# Run differential expression analysis
temp <- results(kidney, contrast = c("age", "1", "27") )
R1_vs_R2 <- formatDESeq2Results(temp)
...
```{r clusterprofiler_organ_tryit-1, exercise=TRUE, echo=FALSE}
#WHAT ORGAN ARE YOU WORKING ON?
```
```{r clusterprofiler_organ_tryit-2, exercise=TRUE, echo=FALSE}
#WHAT ORGAN ARE YOU WORKING ON?
```

```{r clusterprofiler_organ_tryit-3, exercise=TRUE, echo=FALSE}
#WHAT ORGAN ARE YOU WORKING ON?
```

### plotAcrossRegions for the mouse organ dataset

- Multiple functions are available to compare the variation of a gene's expression across different organs
- MGI IDs take the format ENSMUSG###########

#### plotAcrossBrain

- Compares gene expression of the brain across mice of different sex and ages

```{r plotAcrossBrain_1, exercise=TRUE, echo=FALSE}
plotAcrossBrain("ENSMUSG###########")
```
```{r plotAcrossBrain_2, exercise=TRUE, echo=FALSE}
plotAcrossBrain("ENSMUSG###########")
```
```{r plotAcrossBrain_3, exercise=TRUE, echo=FALSE}
plotAcrossBrain("ENSMUSG###########")
```
...
- Compares gene expression of the heart across mice of different sex and ages

```{r plotAcrossHeart_1, exercise=TRUE, echo=FALSE}
plotAcrossHeart("ENSMUSG###########")
```
```{r plotAcrossHeart_2, exercise=TRUE, echo=FALSE}
plotAcrossHeart("ENSMUSG###########")
```
```{r plotAcrossHeart_3, exercise=TRUE, echo=FALSE}
plotAcrossHeart("ENSMUSG###########")
```
...
- Compares gene expression of the kidney across mice of different sex and ages

```{r plotAcrossKidney_1, exercise=TRUE, echo=FALSE}
plotAcrossKidney("ENSMUSG###########")
```
```{r plotAcrossKidney_2, exercise=TRUE, echo=FALSE}
plotAcrossKidney("ENSMUSG###########")
```
```{r plotAcrossKidney_3, exercise=TRUE, echo=FALSE}
plotAcrossKidney("ENSMUSG###########")
```


## Meet the gut-brain dataset


These data are from the [2019 study by Sharon et al. titled **Human Gut Microbiota from Autism Spectrum Disorder Promote Behavioral Symptoms in Mice**](https://pmc.ncbi.nlm.nih.gov/articles/PMC6993574/). We will refer to these data as the gut-brain dataset, since the study focused on the gut-brain axis. To best make use of this data we recommend you complete the following modules: 
...
Although drastically different in overall shape and size, the human and mouse brain are remarkably similar in structure and organization and show homology in neural pathways. Specific parts of the brain are known to be responsible for a specific function. For example, the amygdala is known to control emotions. Despite advancements in our understanding of the brain there is still much we don't yet know about what each region does or how they interact with one another, let alone how they connect to different disease states or developmental pathways.
...
### The mouse gut-brain study design


![](images/sharon-study-design.png){width=100%}
<p class=caption> **Study design from Sharon et al. (2019)**: Germ-free mice were transplated with gut microbiomes from human children and bred. Their offspring were then harvested for brain tissue for RNA-seq analysis. Note that this is a subset of a larger dataset. </p>

These data are from [Sharon et al.'s 2019 study titled Human Gut Microbiota from Autism Spectrum Disorder Promote Behavioral Symptoms in Mice](https://pubmed.ncbi.nlm.nih.gov/31150625/). Researchers took gut microbiome (fecal) samples from typically developing (TD) human children and implanted them into germ-free mice (mice without a microbiome). This parental generation of mice bred amongst themselves and produced offspring from which brain tissue samples were taken.
...
Our standard RNA-seq miniCURE focuses on the midgut dataset and uses *Drosophila* based resources, such as FlyBase. Since the gut-brain dataset is based on mouse samples we have databases and functions specifically for mice that you should use instead:
...
- **Use the specialized functions for the mouse dataset**: This includes getClusterProfilerGenesMouse, plotAcrossTissue, plotAcrossHostDonor, and plotAcrossMice. These functions should work seamlessly with the mouse gut-brain data with no changes in how the code looks to you. 

### Some research ideas to get you started

You will need to identify genes of interest, formulate a research question around them, and check that they are in the gut-brain dataset.
...
### Differential expression template (Mouse gut-brain dataset)
...
raw_results <- results(gutbrain, contrast = c("tissue", "PFC", "STR") )
R1_vs_R2 <- formatDESeq2Results(raw_results)
sig_genes <- filter(R1_vs_R2, padj <= 0.05)
dim(sig_genes)
sig_genes
...
### Get differential expression results for a single gene in the mouse gut-brain dataset

```{r filter-by-name-mouse, exercise=TRUE, echo=FALSE}
raw_results <- results(gutbrain, contrast = c("tissue", "PFC", "STR") )
R1_vs_R2 <- formatDESeq2Results(raw_results)
my_gene <- filter(R1_vs_R2, GeneID=="ENSMUSG###########")
my_gene
...
```{r filter-by-name-mouse_tryit_1, exercise=TRUE, echo=FALSE}

```
```{r filter-by-name-mouse_tryit_2, exercise=TRUE, echo=FALSE}

```
```{r filter-by-name-mouse_tryit_3, exercise=TRUE, echo=FALSE}

```


### clusterProfiler for the mouse gut-brain dataset
...
### plotAcrossRegions for the mouse gut-brain dataset
...
- **REGION2**: Put the other group you're interested in here. <mark>This region is the baseline condition</mark>.
- ***padj***: Use this to control what (modified) p-value you are using to filter out genes. At default, the code shows only genes that have a padj of 0.05 or below. A lower padj is a more stingent, or strict cutoff - the lower the padj, the more sigificantly differentially expressed the genes are. To see all genes, set padj in the code to 1.
...
```{r filter-by-name-drosophila-deseq2, exercise=TRUE, echo=FALSE}
raw_results <- results(midgut, contrast = c("condition", "REGION1", "REGION2") )
R1_vs_R2 <- formatDESeq2Results(raw_results)
...
```{r filter-by-name-drosophila_tryit_4, exercise=TRUE, echo=FALSE}

```
```{r filter-by-name-drosophila_tryit_5, exercise=TRUE, echo=FALSE}

```
```{r filter-by-name-drosophila_tryit_6, exercise=TRUE, echo=FALSE}

```

### Differential expression template (Organ datasets)

```{r deseq2-brain-tryit, exercise=TRUE, echo=FALSE}
#FOR THE BRAIN
temp <- results(brain, contrast = c("age", "1", "27") )
R1_vs_R2 <- formatDESeq2Results(temp)
sig_genes <- filter(R1_vs_R2, padj <= 0.05)
...
```{r deseq2-heart-tryit, exercise=TRUE, echo=FALSE}
#FOR THE HEART
temp <- results(heart, contrast = c("age", "1", "27") )
R1_vs_R2 <- formatDESeq2Results(temp)
sig_genes <- filter(R1_vs_R2, padj <= 0.05)
...
```{r deseq2-kidney-tryit, exercise=TRUE, echo=FALSE}
#FOR THE KIDNEY
temp <- results(kidney, contrast = c("age", "1", "27") )
R1_vs_R2 <- formatDESeq2Results(temp)
sig_genes <- filter(R1_vs_R2, padj <= 0.05)
...
```{r deseq2-organs_tryit4, exercise=TRUE, echo=FALSE}
#WHAT ORGAN ARE YOU WORKING ON?
```
```{r deseq2-organs_tryit5, exercise=TRUE, echo=FALSE}
#WHAT ORGAN ARE YOU WORKING ON?
```
```{r deseq2-organs_tryit6, exercise=TRUE, echo=FALSE}
#WHAT ORGAN ARE YOU WORKING ON?
```


### Get differential expression results for a single gene in a selected organ dataset

```{r filter-by-name-brain-deseq2, exercise=TRUE, echo=FALSE}
#BRAIN
temp <- results(brain, contrast = c("age", "1", "27") )
R1_vs_R2 <- formatDESeq2Results(temp)
...
my_gene
```
```{r filter-by-name-heart-deseq2, exercise=TRUE, echo=FALSE}
#HEART
temp <- results(heart, contrast = c("age", "1", "27") )
R1_vs_R2 <- formatDESeq2Results(temp)
...
my_gene
```
```{r filter-by-name-kidney-deseq2, exercise=TRUE, echo=FALSE}
#KIDNEY
temp <- results(kidney, contrast = c("age", "1", "27") )
R1_vs_R2 <- formatDESeq2Results(temp)
...
my_gene
```
```{r deseq2-organs_tryit7, exercise=TRUE, echo=FALSE}
#WHAT ORGAN ARE YOU WORKING ON?
```
```{r deseq2-organs_tryit8, exercise=TRUE, echo=FALSE}
#WHAT ORGAN ARE YOU WORKING ON?
```
```{r deseq2-organs_tryit9, exercise=TRUE, echo=FALSE}
#WHAT ORGAN ARE YOU WORKING ON?
```
...
```{r filter-by-name-mousebrain-deseq2, exercise=TRUE,echo=FALSE}
raw_results <- results(gutbrain, contrast = c("tissue", "PFC", "STR") )
R1_vs_R2 <- formatDESeq2Results(raw_results)
my_gene <- filter(R1_vs_R2, GeneID=="ENSMUSG###########")
my_gene
...
```{r filter-by-name-mouse_tryit_4, exercise=TRUE, echo=FALSE}

```
```{r filter-by-name-mouse_tryit_5, exercise=TRUE, echo=FALSE}

```
```{r filter-by-name-mouse_tryit_6, exercise=TRUE, echo=FALSE}
...
- **Why is this important**: We can use this analysis to get genes of interest and also get a rough idea of how many related genes are also differentially expressed. For example, a pathway where only 1% of genes are differentially expressed is likely not as important as a pathway where 30% of genes are differentially expressed.
...
### clusterProfiler for the Drosophila midgut dataset
...
```{r clusterprofiler_organ_brain-try_it, exercise=TRUE, echo=FALSE}
#BRAIN
# Run differential expression analysis
temp <- results(brain, contrast = c("age", "1", "27") )
R1_vs_R2 <- formatDESeq2Results(temp)
...
getClusterProfilerGenesMouse(R1_vs_R2_clusters, "YOUR CATEGORY HERE")
```
```{r clusterprofiler_organ_heart-try_it, exercise=TRUE, echo=FALSE}
#HEART
# Run differential expression analysis
temp <- results(heart, contrast = c("age", "1", "27") )
R1_vs_R2 <- formatDESeq2Results(temp)
...
```{r clusterprofiler_organ_kidney-try_it, exercise=TRUE, echo=FALSE}
#KIDNEY
# Run differential expression analysis
temp <- results(kidney, contrast = c("age", "1", "27") )
R1_vs_R2 <- formatDESeq2Results(temp)
...
```{r clusterprofiler_organ_tryit-4, exercise=TRUE, echo=FALSE}
#WHAT ORGAN ARE YOU WORKING ON?
```
```{r clusterprofiler_organ_tryit-5, exercise=TRUE, echo=FALSE}
#WHAT ORGAN ARE YOU WORKING ON?
```

```{r clusterprofiler_organ_tryit-6, exercise=TRUE, echo=FALSE}
#WHAT ORGAN ARE YOU WORKING ON?
```
...
### plotAcrossRegions the for organ dataset

- Use the correct function for your organ of choice
- MGI IDs take the format ENSMUSG########### 
...
```{r plotAcrossBrain_4, exercise=TRUE, echo=FALSE}
plotAcrossBrain("ENSMUSG###########")
```
```{r plotAcrossBrain_5, exercise=TRUE, echo=FALSE}
plotAcrossBrain("ENSMUSG###########")
```
```{r plotAcrossBrain_6, exercise=TRUE, echo=FALSE}
plotAcrossBrain("ENSMUSG###########")
```
...
```{r plotAcrossHeart_4, exercise=TRUE, echo=FALSE}
plotAcrossHeart("ENSMUSG###########")
```
```{r plotAcrossHeart_5, exercise=TRUE, echo=FALSE}
plotAcrossHeart("ENSMUSG###########")
```
```{r plotAcrossHeart_6, exercise=TRUE, echo=FALSE}
plotAcrossHeart("ENSMUSG###########")
```
...
```{r plotAcrossKidney_4, exercise=TRUE, echo=FALSE}
plotAcrossKidney("ENSMUSG###########")
```
```{r plotAcrossKidney_5, exercise=TRUE, echo=FALSE}
plotAcrossKidney("ENSMUSG###########")
```
```{r plotAcrossKidney_6, exercise=TRUE, echo=FALSE}
plotAcrossKidney("ENSMUSG###########")
```



### plotAcrossRegions the for mouse gut-brain dataset
...
- NIAID Visual & Medical Arts. (10/7/2024). Lab Mouse. NIAID NIH BIOART Source. bioart.niaid.nih.gov/bioart/283

- NIAID Visual & Medical Arts. (10/7/2024). Mouse Brain. NIAID NIH BIOART Source. bioart.niaid.nih.gov/bioart/369

- NIAID Visual & Medical Arts. (10/7/2024). Human Heart. NIAID NIH BIOART Source. bioart.niaid.nih.gov/bioart/228

- NIAID Visual & Medical Arts. (10/7/2024). Human Kidney. NIAID NIH BIOART Source. bioart.niaid.nih.gov/bioart/229
...
- Schaum, Nicholas, et al. "Ageing hallmarks exhibit organ-specific temporal signatures." Nature 583.7817 (2020): 596-602. 
