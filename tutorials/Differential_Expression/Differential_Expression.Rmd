...
Use the `results()` and `formatDESeq2Results()` commands in the box below to look at a **comparison between region "a1" and region "Cu"**. HINT: replace "REGION1" and "REGION2" with the regions you want to compare.
...
Use the box below to look at FBgn0000251 and compare its expression between the **a1 and Cu** regions. HINT: Replace "REGION1" and "REGION2" with the regions you want to compare.

```{r filter-by-name-practice, exercise=TRUE}
temp <- results(midgut, contrast = c("condition", "REGION1", "REGION2") )
R1_vs_R2 <- formatDESeq2Results(temp)

gene251 <- filter(R1_vs_R2, GeneID=="FBgn0000251")
gene251
```
...
Use the box below to find genes that are significantly different between the **a1 and Cu** regions. HINT: replace "REGION1" and "REGION2" with the regions you want to compare.
...
Use the box below to find groups of genes that are significantly different **between the a1 and Cu regions** with the `runClusterProfiler` command.  Then use the `getClusterProfilerGenes` command to find out what genes are in those groups. HINT: Replace "REGION1" and "REGION2" with the regions you want to compare. Until you replace YOUR CATEGORY HERE with a category of interest, no genes will show. Your category should be based on your dotplot.
