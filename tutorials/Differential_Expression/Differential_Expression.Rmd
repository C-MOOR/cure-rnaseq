...
image: "![](images/clusterProfiler_dotplot.png)"
image_caption: "clusterProfiler gene set analysis of differential expression in the *Drosophila* midgut by Katherine Cox. License: [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/)"
author:
...
#Check if we are on SciServer by seeing if the file path to midgut.tsv on SciServer is available.
...
if (file.exists(file_path)) {
  ...
  #If not, assume we are on AnVIL and load in the data through the package MarianesMidgutData
  library("MarianesMidgutData")
  R1_vs_R2_clusters <- a1_vs_p1_clusters
  #enable ragg
...
Since setting up DESeq2 is a little complicated, we've gone ahead and run the commands in the background, so that you can focus on exploring the results and looking for patterns in the data. 
...
1. First, use the `results()` command to get the relevant data.  The `results()` command needs to know which regions you want to compare.  Make sure to store this output in a variable.
  + In the example below, we have named this variable `raw_results` because it's not in the right format yet.
2. Reformat the results to make them more useful.  We've made a special command that takes care of reformatting DESeq2 results, named `formatDESeq2Results()`.  Run this command on your raw_results variable to convert in into a nicely formatted dataframe.  Store the dataframe in a new variable, and give it a descriptive name so you can remember what it is.
  + In the example below, we have named our dataframe `R1_vs_R2`, since it contains the data comparing two regions, region 1 (a1) to region 2 (p1).
...
```{r, echo=TRUE}
raw_results <- results(midgut, contrast = c("condition", "a1", "p1") )
...
Use the `results()` and `formatDESeq2Results()` commands in the box below to look at a **comparison between region "a1" and region "Cu"**.
...
R1_vs_R2
```
```{r deseq-results-hint-1, exercise.eval=FALSE}
Replace "REGION1" and "REGION2" with the regions you want to compare. 
```
```{r deseq-results-hint-2, exercise.eval=FALSE}
Replace "REGION1" with "a1" and "REGION2" with "Cu".
```
```{r deseq-results-hint-3, exercise.eval=FALSE}
Check that "a1" and "Cu" have the correct capitalization and are in double quotes.
```
```{r deseq-results-solution, exercise.eval=FALSE}
raw_results <- results(midgut, contrast = c("condition", "a1", "Cu") )
R1_vs_R2 <- formatDESeq2Results(raw_results)
R1_vs_R2
```
...
my_gene
```
```{r filter-by-name-practice-hint-1, exercise.eval=FALSE}
Replace "REGION1" and "REGION2" with the regions you want to compare. 
```
```{r filter-by-name-practice-hint-2, exercise.eval=FALSE}
Replace "REGION1" with "a1" and "REGION2" with "Cu".
```
```{r filter-by-name-practice-hint-3, exercise.eval=FALSE}
Check that "a1" and "Cu" have the correct capitalization and are in double quotes.
```
```{r filter-by-name-practice-solution, exercise.eval=FALSE}
raw_results <- results(midgut, contrast = c("condition", "a1", "Cu") )
R1_vs_R2 <- formatDESeq2Results(raw_results)
R1_vs_R2
...
```{r, echo=TRUE}
sig_01_genes <- filter(R1_vs_R2, padj <= 0.01)
dim(sig_01_genes)
```
In this case, we get 1798 genes, and we expect 0.01 X 1799 = 18 of them to be false positives.  That's a pretty strong list of genes.  We got rid of 125-18 = 107 false genes (yay!).  On the other hand, we also threw out about 600 real genes (boo!)

If we try a stricter cutoff of 0.001:
```{r, echo=TRUE}
sig_001_genes <- filter(R1_vs_R2, padj <= 0.001)
dim(sig_001_genes)
```
Now we get 1277 genes and we expect 0.001 X 1277 ~ 1 to be a false positive.  So we've basically gotten rid of all false genes, but we've also thrown out ~half of the real genes as compared to using the 0.05 cutoff.
...
dim(sig_001_genes)
```
```{r filter-by-padj-practice-hint-1, exercise.eval=FALSE}
Replace "REGION1" and "REGION2" with the regions you want to compare. 
```
```{r filter-by-padj-practice-hint-2, exercise.eval=FALSE}
Replace "REGION1" with "a1" and "REGION2" with "Cu".
```
```{r filter-by-padj-practice-hint-3, exercise.eval=FALSE}
Check that "a1" and "Cu" have the correct capitalization and are in double quotes.
```
```{r filter-by-padj-practice-hint-4, exercise.eval=FALSE}
Change the p-value to be 0.05 or 0.01
```
```{r filter-by-padj-practice-solution, exercise.eval=FALSE}
raw_results <- results(midgut, contrast = c("condition", "a1", "Cu") )
R1_vs_R2 <- formatDESeq2Results(raw_results)
# Change p-value as needed
sig_001_genes <- filter(R1_vs_R2, padj <= 0.05)
dim(sig_001_genes)
```
...
+ You can adjust how many results appear on the plot using `showCategory`.  It's a good idea to find out how many gene categories you have (using the `dim` command on the results from `clusterProfiler`) and then set `dotplot` to show all of them.  If this is too crowded, you can
    + shrink the font by using a smaller `font.size`
    + change how many characters are displayed before wrapping onto a new line using `label_format`
    + set  `showCategory` to a smaller number to display fewer results
...
dotplot(R1_vs_R2_clusters, showCategory=34, title="a1 vs p1", font.size=10, label_format = 50)
```
...
* Look at the code below and adjust it to **examine the a1 and Cu regions**.
...
```{r clusterProfiler-practice, exercise=TRUE, fig.height=7,  exercise.timelimit=120}
...
```
```{r clusterProfiler-practice-hint-1, exercise.eval=FALSE}
Replace "REGION1" and "REGION2" with the regions you want to compare. 
```
```{r clusterProfiler-practice-hint-2, exercise.eval=FALSE}
Replace "REGION1" with "a1" and "REGION2" with "Cu".
```
```{r clusterProfiler-practice-hint-3, exercise.eval=FALSE}
Check that "a1" and "Cu" have the correct capitalization and are in double quotes.
```
```{r clusterProfiler-practice-solution, exercise.eval=FALSE}
#Perform differential expression analysis
raw_results <- results(midgut, contrast = c("condition", "a1", "Cu") )
R1_vs_R2 <- formatDESeq2Results(raw_results)
sig_genes <- filter(R1_vs_R2, padj <= 0.01)
...
For example, if we wanted to get a list of the genes in the "Fatty acid degradation" group for a1 vs p1, we would use the commands you've already learned to run clusterProfiler, then look at the dotplot and use your results to inform what category you put in the `getClusterProfilerGenes()` command:
...
getClusterProfilerGenes(R1_vs_R2_clusters, "Fatty acid degradation")
```
...
Use the box below to find groups of genes that are significantly different **between the a1 and Cu regions** with the `runClusterProfiler` command.  Then use the `getClusterProfilerGenes` command to find out what genes are in those groups. Until you replace YOUR CATEGORY HERE with a category of interest, no genes will show. Your category should be based on your dotplot.

```{r getClusterProfilerGenes-practice, exercise=TRUE, fig.height=7, exercise.timelimit=120}
...
```
```{r getClusterProfilerGenes-practice-hint-1, exercise.eval=FALSE}
Replace "REGION1" and "REGION2" with the regions you want to compare. 
```
```{r getClusterProfilerGenes-practice-hint-2, exercise.eval=FALSE}
Replace "REGION1" with "a1" and "REGION2" with "Cu".
```
```{r getClusterProfilerGenes-practice-hint-3, exercise.eval=FALSE}
Check that "a1" and "Cu" have the correct capitalization and are in double quotes.
```
```{r getClusterProfilerGenes-practice-hint-4, exercise.eval=FALSE}
Replace "YOUR CATEGORY HERE" with a category of interest that you see in your dotplot. This means you will need to run the code twice. The first time you will run the code to get the dot plot and get the categories. The second time you will change the code and add the category of interest to get the list of genes in that category that are differentially expressed between the two regions you chose.
```
```{r getClusterProfilerGenes-practice-hint-5, exercise.eval=FALSE}
Check that the category name is capitalized correctly. The category name is case sensitive and must be the exact same as in the dot plot!
```
```{r getClusterProfilerGenes-practice-solution, exercise.eval=FALSE}
# Run differential expression analysis
raw_results <- results(midgut, contrast = c("condition", "a1", "Cu") )
R1_vs_R2 <- formatDESeq2Results(raw_results)
sig_genes <- filter(R1_vs_R2, padj <= 0.01)
...
# Get the gene symbols for a group of genes
getClusterProfilerGenes(R1_vs_R2_clusters, "Carbon Metabolism")
```
...
This allows us to see patterns of gene expression across all regions.  In this case, FBgn0000003 is only expressed in the a1 and Fe regions, and looking at the y-axis, to a very low level of expression.
...
raw_results <- results(midgut, contrast = c("condition", "a1", "p1") )
R1_vs_R2 <- formatDESeq2Results(raw_results)
R1_vs_R2_sorted <- arrange(R1_vs_R2, padj)
head(R1_vs_R2_sorted)
```
...
Now we can see that it's generally more highly expressed towards the anterior and is expressed at a low level in posterior regions, but there is also moderate expression in the Fe region.
...
1. Repeat the `runClusterProfiler` analysis using different regions, to get a high level picture of which types of genes are differentially expressed across different regions.  Are you seeing similar results to the paper, or are you seeing something different? You can use FlyBase to get the FlyBase ID for a given gene.
2. Plot out expression of a single gene across *all* regions using the FlyBase ID. Does it show a pattern you'd expect from what you knew already, or not?  Do this for a few different genes from the same category (e.g. sugar metabolism genes) to get a feel for how they're behaving. 
...
Remember to replace REGION1 and REGION2 with your midgut regions of choice.

```{r freeplayd-exercise, exercise=TRUE, exercise.eval=FALSE, fig.height=7, exercise.timelimit=120}
# Base differential expression code
raw_results <- results(midgut, contrast = c("condition", "a1", "p1") )
...
```{r freeplayd2-exercise, exercise=TRUE, exercise.eval=FALSE, fig.height=7, exercise.timelimit=120}
# Base differential expression code + pull out a single gene
raw_results <- results(midgut, contrast = c("condition", "a1", "p1") )
...
```{r freeplayd3-exercise, exercise=TRUE, exercise.eval=FALSE, fig.height=7, exercise.timelimit=120}
# Base differential expression code + filter by padj
raw_results <- results(midgut, contrast = c("condition", "a1", "p1") )
...
```{r freeplayd4-exercise, exercise=TRUE, exercise.eval=FALSE, fig.height=7, exercise.timelimit=120}
# Base differential expression code + ClusterProfiler dotplot
...
```{r freeplayd5-exercise, exercise=TRUE, exercise.eval=FALSE, fig.height=7, exercise.timelimit=120}
# Base differential expression code + ClusterProfiler dotplot + give me gene names from a category
raw_results <- results(midgut, contrast = c("condition", "a1", "p1") )
...
getClusterProfilerGenes(R1_vs_R2_clusters, "Peroxisome")

```

```{r freeplayd6-exercise, exercise=TRUE, exercise.eval=FALSE, fig.height=7, exercise.timelimit=120}
#Plot gene expression of a single gene across all regions
plotAcrossRegions("FBgn0035665")
```
