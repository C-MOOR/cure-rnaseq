...
***Note: Here we are using Gini-Simpson's index, where a HIGHER score on the scale (0-1) means a HIGHER alpha diversity. Some publications may use an alternate version of the index which is read inversely.***
...
Use the code below to plot alpha diversity for each individual (x = "subject"), and color by diet. If you get a warning message, you may ignore it and continue.
...
quiz(caption = "Using plot_richness to plot alpha diversity",
  question("For subject S02, which diet has the HIGHEST alpha diveristy?",
    answer("BD", correct = TRUE),
    answer("HD"),
    answer("WO"),
    try_again = "HINT: The answer can be found in the alpha diversity plot where x = subject and color is by diet",
    allow_retry = TRUE
  ),

  question("What male subject has the data point for the LOWEST alpha diversity?",
    answer("S06"),
    answer("S10", correct=TRUE),
    answer("S09"),
    answer("S19"),
    answer("S31"),
    try_again = "HINT: Use subset_samples() to subset males (M instead of F), and specify individuals (subject) on x-axis of the alpha diversity plot.",
    allow_retry = TRUE,
...
quiz(caption = "Creating and interpreting a PCoA plot",
    ...
    question("In a PCoA with only data from ASVs with the class Bacteroidia, what is the percent of variance in the dataset explained by principal coordinate 1?",
      answer("22.5%", correct = TRUE),
      answer("12.8%"),
      answer("13.1%"),
      answer("6.2%"),
      answer("11.7%"),
      try_again = "HINT: You will need to change the code subsetting Phylum and Firmicutes to Class and Bacteroidia.",
      allow_retry = TRUE,
      random_answer_order = TRUE
    )
)
...
In this example, we are comparing samples between the groups of the variable diet (BD, HD, and WO). We will then call my_miso_DESeq2 to check that it is a DESeqDataSet object (it will say class: DESeqDataSet).
...
DESeq2 will only give you the results for one comparision at a time. For example, in the variable diet, we have to make a total of three comparisions if we wanted to make every possible comparision.

- BD vs HD
- HD vs WO
- WO vs BD

We'll just choose the last one for now. my_comparison should be a list of strings identical to what we put in ("diet", "WO", "BD")

```{r create_my_comparison, exercise=TRUE, eval=FALSE}
...
quiz(caption = "Using DESeq2 to examine differential abundance - WO vs BD",
    ...
    answer("11"),
    answer("9"),
    try_again = "HINT: This is equal to the number of rows in your results table.",
    allow_retry = TRUE
  ),
  ...
  question("Does ASV 2114 have a greater abundance in WO or BD?",
    answer("BD", correct = TRUE),
    answer("WO"),
    try_again = "HINT: Our baseline condition is BD, so a negative log2foldchange means the other condition has a lower abundance than BD.",
    allow_retry = TRUE
  ),
  

  question("Of all the ASVs found to be differentially abundant between WO and BD, which has the MOST differential abundance between the WO and BD conditions?",
    answer("ASV245", correct = TRUE),
    answer("ASV1248"),
    answer("ASV1693"),
    answer("ASV2074"),
    answer("ASV2114"),
    try_again = "HINT: What ASV has the highest *absolute value* for Log2FoldChange?",
    allow_retry = TRUE
  ),
    ...
    try_again = "HINT: Find this by manually looking through the DESeq2 results table or looking at the x-axis of the differential abundance plot",
    allow_retry = TRUE
  )
) 
``` 
...
To change the comparison, we need to change the code in Step 2!. In the axample above, we compared diet WO with BD. If we now want to compare HD to BD, we can change the my_comparison() portion of the code and swap out "WO" for "HD" so that we can compare the "HD" to the "BD" (baseline) group.
...
quiz(caption = "Using DESeq2 to examine differential abundance between HD vs BD",
    ...
    allow_retry = TRUE,
    try_again = "HINT: Change WO to HD in step 2 of the above code",
    random_answer_order = TRUE
  ),
    ...
    answer("BD"),
    try_again = "HINT: Our baseline condition here is BD, so an ASV with a postive log2foldChange is higher in the other condition",
    allow_retry = TRUE
  )
) 
```
...
How can we change the factor we are examining for differential abundance. For example, instead of comparing different diets BD, HD, WO we did above, how can we instead perform differential abundance based on subjects?

If we want to change the factor we are looking at (example: from diet to subject) we will have to change the design code in Step 1 as well. In step 1 change diet to subject.

Then of course, step 2 needs to match. In step 2 change diet to subject.
...
quiz(caption = "Using DESeq2 to examine differential abundance",
    ...
    answer("S02"),
    allow_retry = TRUE,
    try_again = "HINT: The baseline condition is listed *second* in step 2.",
    random_answer_order = TRUE
  ),
    ...
    allow_retry = TRUE,
    try_again = "HINT: Confirm that you've made the correct changes in the code as directed in the text in both steps 1 and 2. Then look at the x-axis of the differential abundance plot.",
    random_answer_order = TRUE
  )
)
...
```{r Try_it_plot_richness_subset, exercise=TRUE, echo=FALSE}
...
```{r Try_it_differential_abundance_continuous, exercise=TRUE, echo=FALSE}


```
```{r Try_it_differential_abundance_blank1, exercise=TRUE, echo=FALSE}

```
```{r Try_it_differential_abundance_blank2, exercise=TRUE, echo=FALSE}

```
```{r Try_it_differential_abundance_blank3, exercise=TRUE, echo=FALSE}
...
<!-- plot_bar(miso_glom, x="diet", fill = "Phylum", title = "choose a name for your graph") + -->
